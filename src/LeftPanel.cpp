...
void LeftPanel::draw() {
    // Lewy panel: stały rozmiar, bez titlebara
...
// Szablon pozwala przyjąć zarówno pole typu int, jak i float
template <typename T>
    ...
    T MetricsSample::* field)
{
    ...
    for (int i = start; i < n; ++i) {
        // Pobieramy wartość (int lub float) i rzutujemy na float,
...
void LeftPanel::drawMetricsView() {
    // --- Dodaj wcięcia dla estetyki (zgodnie z poprzednią naprawą) ---
    ImGui::Indent(10.0f);
    ...
    // Prosty pasek postępu pokazujący dominację
    float total = (float)m.alive;
    if (total > 0) {
        ImGui::Text("Dominance:");
        ImGui::SameLine();
        // Malujemy tekst na kolory zwycięzców
        if (m.countPavlov > m.countTitForTat && m.countPavlov > m.countAlwaysD)
            ImGui::TextColored(ImVec4(1, 1, 0, 1), "PAVLOV"); // Żółty
    ...
    // Helper fillSeriesWindowed masz zdefiniowany w tym pliku wcześniej
    ...
    // Aby wykresy miały tę samą skalę Y, musimy znać max populację (zazwyczaj GRID_W * GRID_H)
    ...
    // --- WYKRES 4: ŻÓŁCI (Pavlov) ---
    ImGui::PushStyleColor(ImGuiCol_PlotLines, ImVec4(1.0f, 1.0f, 0.2f, 1.0f)); // Żółty
